...
class MultiLimitOrderServicePerformanceTest: AbstractPerformanceTest() {
    ...
    fun testMatchWithLimitOrderForAllFunds(): Double {
        ...
        primaryOrdersDatabaseAccessor.addLimitOrder(MessageBuilder.buildLimitOrder(walletId = client, assetId = "BTCEUR", price = 4722.0, volume = 0.14825226))
        initServices()

        counter.executeAction { multiLimitOrderService.processMessage(buildMultiLimitOrderWrapper(pair = "BTCEUR", walletId = marketMaker, orders =
    ...
    fun testMatchWithNotEnoughFundsOrder1(): Double {
        ...
        val order = MessageBuilder.buildLimitOrder(walletId = client, assetId = "EURUSD", price = 1.2, volume = 1.0)
        order.reservedLimitVolume = BigDecimal.valueOf(1.19)
        primaryOrdersDatabaseAccessor.addLimitOrder(order)

        initServices()

        counter.executeAction { multiLimitOrderService.processMessage(buildMultiLimitOrderWrapper(walletId = marketMaker, pair = "EURUSD",
