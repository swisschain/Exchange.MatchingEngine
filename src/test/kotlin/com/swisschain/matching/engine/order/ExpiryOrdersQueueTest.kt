...
class ExpiryOrdersQueueTest {
    ...
    fun getExpiredOrdersExternalIds() {
        ...
        assertTrue(queue.getExpiredOrdersExternalIds(date("2018-03-20 10:00"))[DEFAULT_BROKER]!!.isEmpty())
        assertEquals(listOf("1"), queue.getExpiredOrdersExternalIds(date("2018-03-20 10:10"))[DEFAULT_BROKER])
        assertEquals(listOf("1"), queue.getExpiredOrdersExternalIds(date("2018-03-20 10:11"))[DEFAULT_BROKER])
        assertEquals(listOf("1"), queue.getExpiredOrdersExternalIds(date("2018-03-20 20:09"))[DEFAULT_BROKER])
        assertEquals(listOf("1", "2", "3"), queue.getExpiredOrdersExternalIds(date("2018-03-20 20:11"))[DEFAULT_BROKER]!!.sorted())
        assertEquals(listOf("1", "2", "3", "4"), queue.getExpiredOrdersExternalIds(date("2018-03-22 00:00"))[DEFAULT_BROKER]!!.sorted())
    }
    ...
    fun removeIfOrderHasExpiryTime() {
        ...
        assertEquals(listOf(order.externalId), queue.getExpiredOrdersExternalIds(date("2018-03-20 10:11"))[DEFAULT_BROKER])
        assertTrue(queue.removeIfOrderHasExpiryTime(order))
        assertTrue(queue.getExpiredOrdersExternalIds(date("2018-03-20 10:11"))[DEFAULT_BROKER]!!.isEmpty())
    }
