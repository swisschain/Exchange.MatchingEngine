...
class CashInOutOperationServiceTest : AbstractTest() {
    ...
    fun testSmallCashIn() {
        ...
        assertEquals("Client1", cashInTransaction.walletId)
    ...
    fun testSmallReservedCashIn() {
        ...
        assertEquals("Client3", operation.walletId)
    ...
    fun testCashOutNegative() {
        ...
        assertEquals("Client1", cashOutTransaction.walletId)
    ...
    @Test
    fun testRounding() {
        balancesHolder.insertOrUpdateWallets(listOf(Wallet(DEFAULT_BROKER, "Client1", listOf(AssetBalance(DEFAULT_BROKER, "Client1","Asset1", BigDecimal.valueOf(29.99))))), null)
        cashInOutOperationService.processMessage(messageBuilder.buildCashInOutWrapper("Client1", "Asset1", -0.01))
        val balance = testWalletDatabaseAccessor.getBalance("Client1", "Asset1")
    ...
    fun testCashOutFee() {
        ...
        initServices()
        cashInOutOperationService.processMessage(messageBuilder.buildCashInOutWrapper("Client1", "Asset5", -1.0,
                fees = buildFeeInstructions(type = FeeType.CLIENT_FEE, size = 0.05, sizeType = FeeSizeType.ABSOLUTE, targetWalletId = "Client3", assetIds = listOf("Asset4"))))

        assertEquals(BigDecimal.valueOf(0.01), balancesHolder.getBalance(DEFAULT_BROKER, "Client1", "Asset4"))
        assertEquals(BigDecimal.valueOf(0.05), balancesHolder.getBalance(DEFAULT_BROKER, "Client3", "Asset4"))
        assertEquals(BigDecimal.valueOf(10.0), balancesHolder.getBalance(DEFAULT_BROKER, "Client1", "Asset5"))
    }
    ...
    fun testCashOutInvalidFee() {
        ...
        // Negative fee size
        cashInOutOperationService.processMessage(messageBuilder.buildCashInOutWrapper("Client1", "Asset5", -1.0,
                fees = buildFeeInstructions(type = FeeType.CLIENT_FEE, size = -0.1, sizeType = FeeSizeType.PERCENTAGE, targetWalletId = "Client3")))

        assertEquals(BigDecimal.valueOf(3.0), balancesHolder.getBalance(DEFAULT_BROKER, "Client1", "Asset5"))
        assertEquals(BigDecimal.ZERO, balancesHolder.getBalance(DEFAULT_BROKER, "Client3", "Asset5"))
        ...
        cashInOutOperationService.processMessage(messageBuilder.buildCashInOutWrapper("Client1", "Asset5", -0.9,
                fees = buildFeeInstructions(type = FeeType.CLIENT_FEE, size = 0.91, sizeType = FeeSizeType.ABSOLUTE, targetWalletId = "Client3")))

        // Multiple fee amount is more than operation amount
        cashInOutOperationService.processMessage(messageBuilder.buildCashInOutWrapper("Client1", "Asset5", -1.0,
                fees = listOf(buildFeeInstruction(type = FeeType.CLIENT_FEE, size = 0.5, sizeType = FeeSizeType.PERCENTAGE, targetWalletId = "Client3")!!,
                        buildFeeInstruction(type = FeeType.CLIENT_FEE, size = 0.51, sizeType = FeeSizeType.PERCENTAGE, targetWalletId = "Client3")!!)))
