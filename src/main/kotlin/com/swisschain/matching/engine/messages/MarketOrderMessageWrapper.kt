...
class MarketOrderMessageWrapper(
        ...
        val callback: StreamObserver<IncomingMessages.MarketOrderResponse>?,
    ...
    override fun writeResponse(responseBuilder: MessageOrBuilder) {
        if (responseBuilder is IncomingMessages.MarketOrderResponse.Builder) {
    ...
    private fun writeClientResponse(message: IncomingMessages.MarketOrderResponse) {
        if (callback != null) {
            try {
