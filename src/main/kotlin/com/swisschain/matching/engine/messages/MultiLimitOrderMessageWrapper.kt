...
class MultiLimitOrderMessageWrapper(
        ...
        val callback: StreamObserver<IncomingMessages.MultiLimitOrderResponse>?,
    ...
    override fun writeResponse(responseBuilder: MessageOrBuilder) {
        if (responseBuilder is IncomingMessages.MultiLimitOrderResponse.Builder) {
    ...
    private fun writeClientResponse(message: IncomingMessages.MultiLimitOrderResponse) {
        if (callback != null) {
            try {
