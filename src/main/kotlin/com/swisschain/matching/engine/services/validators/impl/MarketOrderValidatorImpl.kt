...
@Autowired constructor(private val assetsPairsHolder: AssetsPairsHolder,
    ...
    private fun isVolumeValid(order: MarketOrder) {
        ...
        if (!OrderValidationUtils.checkMinVolume(order, assetsPairsHolder.getAssetPair(order.brokerId, order.assetPairId))) {
    ...
    private fun isVolumeAccuracyValid(order: MarketOrder) {
        val baseAssetVolumeAccuracy = assetsHolder.getAsset(order.brokerId, getBaseAsset(order)).accuracy
    ...
    private fun getAssetPair(order: MarketOrder) = assetsPairsHolder.getAssetPair(order.brokerId, order.assetPairId)
}