...
import com.swisschain.matching.engine.order.process.context.LimitOrderExecutionContext
...
class LimitOrderProcessor(private val limitOrderInputValidator: LimitOrderInputValidator,
    ...
    private fun performInputValidation(orderContext: LimitOrderExecutionContext): OrderValidationResult {
        ...
        try {
            limitOrderInputValidator.validateLimitOrder(applicationSettingsHolder.isTrustedClient(order.walletId),
                    order,
                    assetPair,
                    order.assetPairId,
                    baseAsset)
    ...
    private fun performBusinessValidation(orderContext: LimitOrderExecutionContext): OrderValidationResult {
        val order = orderContext.order
        try {
            limitOrderBusinessValidator.performValidation(applicationSettingsHolder.isTrustedClient(order.walletId),
                    order,
                    orderContext.availableLimitAssetBalance!!,
                    orderContext.limitVolume!!,
                    orderContext.executionContext.orderBooksHolder.getChangedCopyOrOriginalOrderBook(order.brokerId, order.assetPairId),
                    orderContext.executionContext.date,
                    orderContext.executionContext.getOrderBookTotalSize(order.brokerId))
        } catch (e: OrderValidationException) {
            return OrderValidationResult(false, false, e.message, e.orderStatus)
        }
        return OrderValidationResult(true)
    ...
    private fun processValidOrder(orderContext: LimitOrderExecutionContext): ProcessedOrder {
        val order = orderContext.order
        val orderBook = orderContext.executionContext.orderBooksHolder.getChangedCopyOrOriginalOrderBook(order.brokerId, order.assetPairId)
