...
import com.swisschain.matching.engine.order.process.context.PreviousLimitOrdersContext
...
class PreviousLimitOrdersProcessor(private val genericLimitOrderService: GenericLimitOrderService,
    ...
    fun cancelAndReplaceOrders(brokerId: String,
        ...
        val context = PreviousLimitOrdersContext(brokerId,
                walletId,
                assetPairId,
        ...
        if (context.cancelAllPreviousLimitOrders) {
            if (context.cancelBuySide) {
                context.buyOrdersToCancel = genericLimitOrderService.searchOrders(brokerId, context.walletId, context.assetPairId, true)
                context.buyStopOrdersToCancel = genericStopLimitOrderService.searchOrders(brokerId, context.walletId, context.assetPairId, true)
            }
            if (context.cancelSellSide) {
                context.sellOrdersToCancel = genericLimitOrderService.searchOrders(brokerId, context.walletId, context.assetPairId, false)
                context.sellStopOrdersToCancel = genericStopLimitOrderService.searchOrders(brokerId, context.walletId, context.assetPairId, false)
            }
        }
    ...
    private fun splitOneSideLimitAndStopOrders(context: PreviousLimitOrdersContext,
        ...
        val ordersToCheck = ordersToCancel
                ?: genericLimitOrderService.searchOrders(context.brokerId, context.walletId, context.assetPairId, isBuy)
        ...
        val stopOrdersToCheck = stopOrdersToCancel
                ?: genericStopLimitOrderService.searchOrders(context.brokerId, context.walletId, context.assetPairId, isBuy)
        splitOneSideOrders(context,
                isAlreadyCancelled,
                replacementsByPreviousExternalId,
                stopOrdersToCheck,
